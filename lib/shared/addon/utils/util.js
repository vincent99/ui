...
export function formatIB(bytes) {
  return formatSi(bytes, 1024, "iB", "B");
}

export function formatKiB(kibibytes) { // 1024 bytes per kibibyte
  return formatSi(kibibytes, 1024, "iB", "B", 1);
}

export function formatMiB(mibibytes) { // 1,048,576 btyes per mibibyte
  return formatSi(mibibytes, 1024, "iB", "B", 2);
}

export function formatB(bytes) {
  return formatSi(bytes, 1000, "iB");
}

export function formatKB(kilobytes) { // 1,000 bytes per kilobyte
  return formatSi(kilobytes, 1000, "B", null, 1);
}

export function formatMB(megabtyes) { // 1,000,000 bytes per megabyte
  return formatSi(megabtyes, 1000, "B", null, 2);
}

export function formatKbps(kbits) {
  return formatSi(kbits, 1000, "bps", "Bps", 1);
}
